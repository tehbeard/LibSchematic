...
/**
 * <p>
 * A SchematicFactory processes {@link SchematicFile}s , passing them to
 * multiple {@link AbstractSchematicWorker}s.<br/>
 * These workers manipulate a Schematic, such as changing the owner of a block,
 * removing blocks or rotating a schematic.<br/>
 * The schematic passed into a SchematicFactory is copied before use, preserving
 * the value of the original, additionally {@link AbstractSchematicWorker}s<br/>
 * may generate new a {@link SchematicFile} and pass this along the chain (an
 * example would be the rotation worker, which writes a new schematic based on<br/>
 * rotating the old one.
 * </p>
 * <p>
 * The lifecycle of a SchematicFactory is as follows:<br/>
 * new() -> loadWorkers()... -> loadSchematic() -> produce({@link IFactoryOuput}
 * )<br/>
 * Alternatively, in a scenario where you apply the same workers to multiple
 * schematics:<br/>
 * 
 * <pre>
...
public class SchematicFactory {
    ...
    private SchematicFile process(SchematicFile file) {
        try {
            ...
            System.out
                    .println("An error occured while processing the schematic");
            e.printStackTrace();
            return null;
        }
     ...
     * 
     * @param workers
     *            {@link AbstractSchematicWorker}s to add
     * @return this {@link SchematicFactory} to allow method linking
     */
