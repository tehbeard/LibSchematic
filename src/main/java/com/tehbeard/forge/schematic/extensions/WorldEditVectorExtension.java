...
/**
 * Adds access to WorldEdit's offset and origin vectors. These vectors can be
 * used by a SchematicWorker to place a schematic in a more intuitive place.
...
public class WorldEditVectorExtension implements SchematicExtension {
    ...
    public void onLoad(NBTTagCompound tag, SchematicFile file) {
        origin = new SchVector(tag.getInteger("WEOriginX"),
                tag.getInteger("WEOriginY"), tag.getInteger("WEOriginZ"));

        offset = new SchVector(tag.getInteger("WEOffsetX"),
                tag.getInteger("WEOffsetY"), tag.getInteger("WEOffsetZ"));
    }
