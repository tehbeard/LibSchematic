...
/**
 * An implementation of a schematic rotation handler That uses an array lookup
 * table + bitmask as the data for rotation For simple mod blocks this can be
 * used in lieu of a custom {@link SchematicDataHandler} More complex handlers
 * that implement several interfaces can use this class via delegation
 * 
 * @author James
 * 
 */
public class ArrayRotationHandler extends SimpleRotationHandler {
     ...
     * 
     * @param mask
     *            mask for data that is NOT rotation bits i.e. if top two bits
     *            are not rotation data, then the mask is 0xC
     * @param rotationsArray
     *            array of ints to rotate around
     */
